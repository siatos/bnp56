...
subplot(1, 6, 1), imshow(CT_img), title('Initial image CT scan')
subplot(1, 6, 2), imshow(mask_img), title('Mask Image for Region Isolation')
subplot(1, 6, 3), imshow(ANDimg), title('Isolate Region using AND with Mask')
subplot(1, 6, 4), imshow(NORMimg), title('Normalized Isolated Region')
subplot(1, 6, 5), imshow(ANDINVimg), title('AND Inverse Mask ')
subplot(1, 6, 6), imshow(maxout_img), title('MAX out image')
